...
echo "<html><p>Autoscaling</p><p>"`cat /etc/hostname`"</p><img src="https://${sbercloud_obs_bucket_object.netology-logo.bucket}.obs.ru-moscow-1.hc.sbercloud.ru/${sbercloud_obs_bucket_object.netology-logo.key}" alt="Netology logo"></html>" > /var/www/html/index.html
EOF
  }
}
